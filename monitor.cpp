...
void convertCelsiusToFarenheit(HealthParameter& temperaturearameter) {
  if(temperaturearameter.vitalType == VitalType::TEMPERATURE && temperaturearameter.unit == VitalUnit::CELSIUS) {
    temperaturearameter.vitalValue = (temperaturearameter.vitalValue * 9.0f / 5.0f) + 32;
    temperaturearameter.unit = VitalUnit::FARENHEIT;
  }
}

void createHealthParameterRanges( const vector<HealthParameterLimitsAndTolerence>& healthParamLimitsNTolerence)
{
  for(int i = 0; i < healthParamLimitsNTolerence.size(); i++) {
    switch(healthParamLimitsNTolerence[i].vitalType) {
      case VitalType::TEMPERATURE: {
        ...
        float tolerance = healthParamLimitsNTolerence[i].upperLimit * healthParamLimitsNTolerence[i].warningTolerance;
        temperatureParamLimits.warningLowerLimit = healthParamLimitsNTolerence[i].lowerLimit + tolerance;
        temperatureParamLimits.normallimit = healthParamLimitsNTolerence[i].upperLimit - tolerance;
      ...
      case VitalType::PULSE_RATE: {
        ...
        float tolerance = healthParamLimitsNTolerence[i].upperLimit * healthParamLimitsNTolerence[i].warningTolerance;
        pulseRateParamLimits.warningLowerLimit = healthParamLimitsNTolerence[i].lowerLimit + tolerance;
...
int vitalsOk(vector<HealthParameter> _healthParams) {
  for (int i = 0; i < _healthParams.size(); i++) {
    VITAL_RANGE_CLASSIFICATION vitalCondition = getVitalCondition(_healthParams[i], getHealthParamLimits(_healthParams[i].vitalType));
